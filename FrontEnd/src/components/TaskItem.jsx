...
const TaskItem = ({task,  onRefresh, onLogout, showCompleteCheckbox = true}) => {
  ...
  const [subtasks, setSubtasks] = useState(task.subtasks || [])
    ...
    const handleComplete = async () => {
      ...
      try{
        await axios.put(`${import.meta.env.VITE_API_URL}/api/tasks/${task._id}/ct`, 
          {completed: newStatus},
...
const handleDelete = async () => {
  try{
    await axios.delete(`${import.meta.env.VITE_API_URL}/api/tasks/${task._id}/ct`,{
      headers: getAuthHeaders()
    })
     onRefresh?.() 
  }catch(err){
  ...
  const handleSave = async (updatedTask) => {
    try {
      ...
      await axios.put(`${API_BASE}/${task._id}/ct`, payload, { headers: getAuthHeaders() })

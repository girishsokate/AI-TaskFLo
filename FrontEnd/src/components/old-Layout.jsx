...
const Layout = ({ user, onLogout }) => {
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      const { data } = await axios.get("http://localhost:4000/api/tasks/ct", {
  ...
  if (error)
    return (
      <div className="min-h-screen bg-gray-50 p-6 flex items-center justify-center">
        <div className="bg-red-50 text-red-600 p-4 rounded-xl flex items-center flex-col border border-red-100 max-w-md">
          <p className="font-medium mb-2">Error loading tasks</p>
          <p className="text-sm">{error}</p>
          <button
            onClick={fetchTasks}
            className="mt-4 px-4 py-2 bg-red-100 text-red-700 items-center rounded-lg text-sm font-medium hover:bg-red-200 transition-colors"
          >
            Try Again
          </button>
        </div>
